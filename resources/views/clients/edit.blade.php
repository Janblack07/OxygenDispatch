                    ...
                    <form method="POST" action="{{ route('clients.update', $client) }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="name" value="{{ old('name', $client->name) }}" class="form-input-sm" required>
                            </div>

                            <div>
                                <label class="form-label-sm">Documento</label>
                                <input name="document" value="{{ old('document', $client->document) }}" class="form-input-sm">
                            </div>

                            <div>
                                <label class="form-label-sm">Teléfono</label>
                                <input name="phone" value="{{ old('phone', $client->phone) }}" class="form-input-sm">
                            </div>

                            <div>
                                <label class="form-label-sm">Email</label>
                                <input type="email" name="email" value="{{ old('email', $client->email) }}" class="form-input-sm">
                            </div>

                            <div class="md:col-span-2">
                                <label class="form-label-sm">Dirección</label>
                                <input name="address" value="{{ old('address', $client->address) }}" class="form-input-sm">
                            </div>
                        </div>
