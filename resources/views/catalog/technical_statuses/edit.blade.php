<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h2 class="font-semibold text-lg text-gray-800 leading-tight">{{ __('Editar estado técnico') }}</h2>
                <p class="mt-0.5 text-xs text-gray-500">Actualiza el catálogo.</p>
            </div>

            <a href="{{ route('technical-statuses.index') }}"
    ...
    <div class="py-6">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-4">
                    <form method="POST" action="{{ route('technical-statuses.update', $item) }}" class="space-y-4">
                        @csrf
                        @method('PUT')
